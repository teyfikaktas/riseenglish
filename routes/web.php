...
Route::get('/hakanhoca', [ExamController::class, 'publicReportIndex'])
    ->name('public.report-index');

Route::get('/hakanhoca/rapor', [ExamController::class, 'publicTodayReport'])
    ->name('public.today-report');

Route::get('/hakanhoca/rapor/{date}', [ExamController::class, 'publicDailyReport'])
    ->name('public.daily-report');
// ============================================================
// PUBLIC ROUTES
// ============================================================
...
Route::get('/oturum-ac', function () {
    if (Auth::check()) {
        if (Auth::user()->hasRole('yonetici')) return redirect('/admin/dashboard');
        if (Auth::user()->hasRole('ogretmen')) return redirect('/ogretmen/panel');
        return redirect('/');
    }
...
Route::get('/gizlilik-politikasi', fn () => view('privacy.privacy'))->name('privacy');
...
Route::get('/belge/{token}', [App\Http\Controllers\PublicDocumentController::class, 'show'])->name('public.document.show');
...
// SMS Password Reset (Guest)
Route::middleware('guest')->group(function () {
    Route::get('forgot-password-sms', [App\Http\Controllers\Auth\SmsPasswordResetController::class, 'showForgotForm'])->name('password.sms.request');
    Route::post('forgot-password-sms', [App\Http\Controllers\Auth\SmsPasswordResetController::class, 'sendResetLink'])->name('password.sms.email');
    Route::get('reset-password-sms/{token}', [App\Http\Controllers\Auth\SmsPasswordResetController::class, 'showResetForm'])->name('password.sms.reset');
    Route::post('reset-password-sms', [App\Http\Controllers\Auth\SmsPasswordResetController::class, 'reset'])->name('password.sms.update');
});

// Word Set PDF Exports (no auth required)
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('/home', function () {
        if (Auth::user()->hasRole('yonetici')) return redirect('/admin/dashboard');
        if (Auth::user()->hasRole('ogretmen')) return redirect('/ogretmen/panel');
        if (!Auth::user()->phone_verified) return redirect()->route('verification.phone.notice');
        if (!Auth::user()->teacher_approved) return redirect()->route('waiting-approval');
        return to_route('standard.home');
    })->name('home');

    Route::get('/onay-bekleniyor', fn () => view('auth.waiting-approval'))->name('waiting-approval');
    ...
    Route::post('/telefon-dogrulama/otp', [App\Http\Controllers\OtpController::class, 'verify'])->name('verification.phone.verify');
    Route::post('/telefon-dogrulama/send', [App\Http\Controllers\OtpController::class, 'sendOtp'])->name('verification.phone.send');
...
Route::get('/users/pro-students/{group}/pdf', [AdminUserController::class, 'proStudentsPdf'])->name('users.proStudents.pdf');
        ...
        Route::get('/users/{user}/manage-courses', [AdminUserController::class, 'manageCourses'])->name('users.manageCourses');
        Route::post('/users/{user}/enroll-course', [AdminUserController::class, 'enrollCourse'])->name('users.enrollCourse');
...
Route::post('/users/{user}/unverify-phone', [AdminUserController::class, 'unverifyPhone'])->name('users.unverifyPhone');
        ...
        Route::put('/users/{user}/membership/{membership}', [AdminUserController::class, 'updateMembership'])->name('users.updateMembership');
        Route::delete('/users/{user}/membership/{membership}', [AdminUserController::class, 'deleteMembership'])->name('users.deleteMembership');
...
Route::middleware(['auth', 'role:ogretmen', 'verified.phone'])->group(function () {
    Route::prefix('ogretmen')->name('ogretmen.')->group(function () {
        ...
        Route::get('/zinciri-kirma-takip', fn () => view('teacher.chain-breaker-dashboard'))->name('chain-breaker-dashboard');
        ...
        // Private Lessons
        Route::get('/ozel-derslerim', [TeacherPrivateLessonController::class, 'index'])->name('private-lessons.index');
        Route::get('/ozel-ders/takvim', [TeacherPrivateLessonController::class, 'calendar'])->name('private-lessons.calendar');
        Route::get('/ozel-ders/olustur', [TeacherPrivateLessonController::class, 'create'])->name('private-lessons.create');
        Route::post('/ozel-ders/kaydet', [TeacherPrivateLessonController::class, 'store'])->name('private-lessons.store');
        Route::get('/ozel-ders-talep', [TeacherPrivateLessonController::class, 'pendingRequests'])->name('private-lessons.pendingRequests');
        Route::get('/check-lesson-conflict', [TeacherPrivateLessonController::class, 'checkLessonConflictApi'])->name('check.lesson.conflict');
        Route::get('/odevlerim', [TeacherPrivateLessonController::class, 'allHomeworks'])->name('private-lessons.homeworks');

        // Private Lesson Group
        Route::get('/ozel-ders-grup/{id}', [TeacherPrivateLessonController::class, 'showLesson'])->name('private-lessons.showLesson');
        Route::get('/ozel-ders-grup/{id}/duzenle', [TeacherPrivateLessonController::class, 'editLesson'])->name('private-lessons.editLesson');
        Route::put('/ozel-ders-grup/{id}', [TeacherPrivateLessonController::class, 'updateLesson'])->name('private-lessons.updateLesson');
        ...
        // Private Lesson CRUD
        Route::get('/ozel-ders/{id}', [TeacherPrivateLessonController::class, 'show'])->name('private-lessons.show');
        Route::get('/ozel-ders/{id}/duzenle', [TeacherPrivateLessonController::class, 'edit'])->name('private-lessons.edit');
        Route::put('/ozel-ders/{id}', [TeacherPrivateLessonController::class, 'update'])->name('private-lessons.update');
        Route::delete('/ozel-ders/{id}', [TeacherPrivateLessonController::class, 'destroy'])->name('private-lessons.destroy');
        Route::post('/ozel-ders/{id}/onayla', [TeacherPrivateLessonController::class, 'approve'])->name('private-lessons.approve');
        Route::post('/ozel-ders/{id}/reddet', [TeacherPrivateLessonController::class, 'reject'])->name('private-lessons.reject');
        Route::post('/ozel-ders/{id}/tamamla', [TeacherPrivateLessonController::class, 'completeLesson'])->name('private-lessons.complete');
        Route::post('/ozel-ders/{id}/odeme-al', [TeacherPrivateLessonController::class, 'takePayment'])->name('private-lessons.takePayment');
        Route::post('/ozel-ders/{id}/odeme-guncelle', [TeacherPrivateLessonController::class, 'updatePaymentStatus'])->name('private-lessons.updatePaymentStatus');
        ...
        Route::get('/ozel-ders-seans/{id}', [TeacherPrivateLessonController::class, 'showSession'])->name('private-lessons.session.show');
        ...
        Route::post('private-lessons/session/{sessionId}/topics/quick-add', [TeacherPrivateLessonController::class, 'quickAddTopic'])->name('private-lessons.session.topics.quick-add');

        // Homework & Materials
        Route::get('/ozel-ders/{id}/odev-ekle', [TeacherPrivateLessonController::class, 'showAddHomework'])->name('private-lessons.homework.create');
        Route::post('/ozel-ders/{id}/odev-ekle', [TeacherPrivateLessonController::class, 'storeHomework'])->name('private-lessons.homework.store');
        Route::get('/ozel-ders/{id}/odevler', [TeacherPrivateLessonController::class, 'viewHomeworks'])->name('private-lessons.homework.index');
        Route::delete('/ozel-ders-odev/{homeworkId}', [TeacherPrivateLessonController::class, 'deleteHomework'])->name('private-lessons.homework.delete');
        Route::get('/ozel-ders-odev/{homeworkId}/teslimler', [TeacherPrivateLessonController::class, 'viewHomeworkSubmissions'])->name('private-lessons.homework.submissions');
        Route::get('/ozel-ders-odev/{homeworkId}/indir', [TeacherPrivateLessonController::class, 'downloadHomework'])->name('private-lessons.homework.download');
        Route::get('/ozel-ders-teslim/{submissionId}', [TeacherPrivateLessonController::class, 'viewSubmission'])->name('private-lessons.submission.view');
        Route::post('/ozel-ders-teslim/{submissionId}/degerlendir', [TeacherPrivateLessonController::class, 'gradeSubmission'])->name('private-lessons.submission.grade');
        Route::get('/ozel-ders-teslim/{submissionId}/indir', [TeacherPrivateLessonController::class, 'downloadSubmission'])->name('private-lessons.submission.download');
        ...
        Route::get('/ozel-ders/{id}/materyal-ekle', [TeacherPrivateLessonController::class, 'showAddMaterial'])->name('private-lessons.material.create');
        Route::post('/ozel-ders/{id}/materyal-ekle', [TeacherPrivateLessonController::class, 'storeMaterial'])->name('private-lessons.material.store');
        Route::delete('/ozel-ders-materyal/{materialId}', [TeacherPrivateLessonController::class, 'deleteMaterial'])->name('private-lessons.material.delete');
        Route::get('/ozel-ders-materyal/{id}/indir', [TeacherPrivateLessonController::class, 'downloadMaterial'])->name('private-lessons.material.download');
    });
});
...
Route::middleware(['auth', 'role:ogrenci|ogretmen', 'verified.phone', 'teacher.approved'])->group(function () {
    Route::prefix('ogrenci')->name('ogrenci.')->group(function () {
        ...
        Route::get('/word-match-game', fn () => view('game'))->name('word-match-game');
        ...
        // Private Lessons
        Route::get('/ozel-derslerim', [StudentPrivateLessonController::class, 'index'])->name('private-lessons.index');
        Route::get('/tamamlanan-dersler', [StudentPrivateLessonController::class, 'completed'])->name('private-lessons.completed');
        Route::get('/ozel-ders/{id}', [StudentPrivateLessonController::class, 'showLesson'])->name('private-lessons.lesson');
        Route::get('/ozel-ders-seans/{id}', [StudentPrivateLessonController::class, 'showSession'])->name('private-lessons.session');

        // Student Materials & Homeworks
        Route::get('/ders-materyalleri', [StudentPrivateLessonController::class, 'materials'])->name('private-lessons.materials');
        Route::get('/materyal/{id}/indir', [StudentPrivateLessonController::class, 'downloadMaterial'])->name('private-lessons.material.download');
        Route::get('/odevlerim', [StudentPrivateLessonController::class, 'homeworks'])->name('private-lessons.homeworks');
        Route::get('/odev/{id}', [StudentPrivateLessonController::class, 'showHomework'])->name('private-lessons.homework');
        Route::post('/odev/{id}/teslim-et', [StudentPrivateLessonController::class, 'submitHomework'])->name('private-lessons.homework.submit');
        Route::get('/odev-teslim-dosya/{id}/indir', [StudentPrivateLessonController::class, 'downloadSubmission'])->name('private-lessons.submission-file.download');
        Route::delete('/odev-teslim-dosya/{id}/sil', [StudentPrivateLessonController::class, 'deleteSubmissionFile'])->name('private-lessons.submission-file.delete');
        Route::get('/odev-teslim/{id}/indir', [StudentPrivateLessonController::class, 'downloadSubmission'])->name('private-lessons.submission.download');
    });
});