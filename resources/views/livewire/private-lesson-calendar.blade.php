                                        ...
                                        <td class="relative p-2 border-r {{ $date->isToday() ? 'bg-indigo-50/30' : '' }} {{ $isCurrentHour ? 'bg-yellow-50' : '' }}">
                                            @if($hasEvents)
                                                @foreach($calendarData[$dateFormatted][$timeSlot] as $occurrence)
                                                    ...
                                                    <!-- Ders kartı - optimize edilmiş boyut -->
                                                    <div class="mb-2 p-2 rounded-lg text-sm shadow-md border-l-4 {{ $colors['border'] }} bg-white transition-all duration-200 hover:shadow-lg transform hover:-translate-y-1 group"
                                                        ...
                                                        <div onclick="window.location.href='{{ route('ogretmen.private-lessons.session.show', $occurrence['id']) }}'"
                                                            ...
                                                            <div class="flex justify-between items-center mb-1">
                                                                <div class="font-bold {{ $compactView ? 'text-xs' : 'text-sm' }} text-gray-800 truncate flex-1">{{ $occurrence['title'] }}</div>
                                                                <span class="text-xs px-1.5 py-0.5 rounded-full {{ $colors['bg'] }} {{ $colors['text'] }}">
                                                                    @php
                                                                        $statusText = [
                                                                            'scheduled' => 'P',
                                                                            'completed' => 'T',
...
<div class="mt-2 grid grid-cols-2 gap-1">
    @if($occurrence['status'] !== 'completed')
        <button wire:click="completeLesson({{ $occurrence['id'] }})" 
                class="flex items-center justify-center px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs transition-colors duration-200 cursor-pointer">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    @else
        <a href="{{ route('ogretmen.private-lessons.session.createReport', $occurrence['id']) }}"
           class="flex items-center justify-center px-2 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-xs transition-colors duration-200 cursor-pointer">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <a href="{{ route('ogretmen.private-lessons.session.delete', $occurrence['id']) }}"
       class="flex items-center justify-center px-2 py-1 bg-red-600 hover:bg-red-700 text-white rounded text-xs transition-colors duration-200 cursor-pointer">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                      ...
                      <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                          <div class="flex flex-col">
                              ...
                              <a 
                                  href="{{ route('ogretmen.private-lessons.material.create', $selectedLesson['id']) }}" 
