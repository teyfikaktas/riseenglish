    ...
    <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-8">
            ...
            <div class="flex items-center space-x-3">
                ...
                <span class="px-5 py-2.5 bg-indigo-100 text-indigo-800 font-medium rounded-lg border border-indigo-200">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                <label for="teacher" class="block text-sm font-semibold text-gray-700 mb-2">Öğretmen</label>
                <div class="relative">
                    <select id="teacher" wire:change="filterByTeacher($event.target.value)" class="block w-full pl-4 pr-10 py-3 text-base border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-xl shadow-sm appearance-none bg-white">
                        ...
                        @foreach($timeSlots as $timeSlot)
                            <tr class="{{ $loop->even ? 'bg-gray-50' : 'bg-white' }}">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 border-r sticky left-0 bg-indigo-50 z-10">
                                    <div class="flex items-center justify-center {{ $compactView ? 'h-8' : 'h-16' }}">
                                        {{ substr($timeSlot, 0, 5) }}
                                    </div>
                                </td>
                                @foreach($weekDates as $date)
                                    ...
                                    <td class="relative p-2 border-r {{ $date->isToday() ? 'bg-indigo-50/30' : '' }} {{ $isCurrentHour ? 'bg-yellow-50' : '' }}">
                                        @if($hasEvents)
                                            @foreach($calendarData[$dateFormatted][$timeSlot] as $occurrence)
                                                @php
                                                    ...
                                                    $start = Carbon\Carbon::parse($occurrence['start_time'])->format('H:i');
                                                    $end = Carbon\Carbon::parse($occurrence['end_time'])->format('H:i');
                                                @endphp
                                        ...
                                        @else
                                            <div class="flex items-center justify-center w-full h-full">
                                                <div class="{{ $compactView ? 'h-8' : 'h-20' }} w-full border border-dashed border-gray-200 rounded-lg flex items-center justify-center">
