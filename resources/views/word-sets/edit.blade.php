...
<div>
    ...
    <select id="category_id" name="category_id"
        ...
        @foreach($categories as $cat)
            <option value="{{ $cat->id }}" {{ old('category_id', $wordSet->category_id) == $cat->id ? 'selected' : '' }}>
                {{ $cat->name }}
            </option>
            @foreach($cat->children as $child)
                <option value="{{ $child->id }}" {{ old('category_id', $wordSet->category_id) == $child->id ? 'selected' : '' }}>
                    &nbsp;&nbsp;&nbsp;› {{ $child->name }}
                </option>
                @foreach($child->children as $grandchild)
                    <option value="{{ $grandchild->id }}" {{ old('category_id', $wordSet->category_id) == $grandchild->id ? 'selected' : '' }}>
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;› {{ $grandchild->name }}
                    </option>
                @endforeach
            @endforeach
