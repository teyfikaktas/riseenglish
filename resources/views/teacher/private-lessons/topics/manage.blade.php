...
<div class="bg-white rounded-xl shadow-lg p-4 md:p-5 border border-gray-100 max-w-5xl mx-auto">
    <div class="mb-4 flex justify-between items-center">
        <h1 class="text-xl font-bold text-indigo-800">İşlenen Konular</h1>
    ...
    <!-- Açıklama -->
    <div class="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-100 text-blue-800">
        <p class="text-sm">
    ...
    <div class="space-y-4">
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
            <div class="bg-gray-50 px-4 py-2 font-medium text-gray-700 border-b border-gray-200">
                {{ $category->name }}
            </div>
            <div class="p-4">
                    ...
                    <div class="bg-gray-50 rounded-lg p-3 flex justify-between items-center border border-gray-100">
                        <div>
                            <div class="flex items-center">
                                <h5 class="font-medium text-gray-800">{{ $topic->name }}</h5>
                            ...
                            <div class="mt-1 flex items-center">
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-green-600 mr-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                                </svg>
                                @endfor
                                <span class="text-xs text-gray-600">({{ $topicCounts[$topic->id] }} kez işlendi)</span>
                            </div>
                            @endif
                        </div>
...
<button onclick="openTopicNoteModal({{ $topic->id }}, '{{ $topic->name }}')" 
    class="px-2 py-1 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all duration-200 text-xs flex items-center">
...
<span>Ekle</span>
                            ...
                            <form action="{{ route('ogretmen.private-lessons.session.topics.remove', $session->id) }}" method="POST" onsubmit="return confirm('Bu konuyu çıkarmak istediğinize emin misiniz?');">
                                ...
                                <button type="submit" class="px-2 py-1 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all duration-200 text-xs">
...
<div id="topicNoteModal" class="fixed inset-0 flex items-center justify-center z-50 hidden">
    ...
    <div class="bg-white rounded-lg shadow-xl z-10 w-full max-w-md mx-4">
        <div class="flex justify-between items-center border-b border-gray-200 px-4 py-3">
            <h3 class="text-lg font-semibold text-gray-800" id="modalTopicTitle">Konu Notu Ekle</h3>
        ...
        <form id="topicNoteForm" action="{{ route('ogretmen.private-lessons.session.topics.add', $session->id) }}" method="POST">
            ...
            <div class="px-4 py-3">
                <label for="notes" class="block text-sm font-medium text-gray-700 mb-1">İşlenen Konu Hakkında Not</label>
                <textarea id="notes" name="notes" rows="4"
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                          placeholder="Öğrencinin bu konudaki performansı, dikkat edilmesi gereken noktalar, vb."></textarea>
                <p class="text-xs text-gray-500 mt-1">Bu not sadece sizin tarafınızdan görülebilir.</p>
            </div>
            
            <div class="flex justify-end px-4 py-3 bg-gray-50 rounded-b-lg">
                <button type="button" onclick="closeTopicNoteModal()" 
                        class="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 transition-all duration-200 text-sm mr-2">
                    İptal
                </button>
                <button type="submit"
