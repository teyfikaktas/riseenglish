                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex space-x-2">
                                    ...
                                    <form action="{{ route('ogretmen.private-lessons.session.destroy', $session->id) }}"
                                        ...
                                        <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-xs flex items-center">
                                            <i class="fas fa-trash mr-1"></i> Sadece Bu Seansı Sil
                                        </button>
                                    </form>
                                    
                                    <!-- Bu ve sonraki seansları sil -->
                                    <form action="{{ route('ogretmen.private-lessons.session.destroy', $session->id) }}"
                                          method="POST"
                                          onsubmit="return confirm('Bu ve sonraki aynı gün ve saatteki tüm seansları silmek istediğinize emin misiniz?');"
                                          class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <input type="hidden" name="delete_scope" value="all_future">
                                        <button type="submit" class="bg-orange-500 hover:bg-orange-600 text-white px-3 py-1 rounded text-xs flex items-center">
