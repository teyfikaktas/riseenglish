...
<!-- Mobil menü içine Zinciri Kırma bağlantısını ekleyeceğiz -->
    ...
    <div class="py-2 divide-y divide-gray-100">
        ...
        <a href="{{ url('/iletisim') }}" class="flex items-center px-4 py-3 hover:bg-red-50 {{ request()->is('iletisim') ? 'bg-red-50 border-l-4 border-red-500 pl-3' : '' }}">
         ...
         @auth
            ...
            @if(auth()->user()->hasRole('ogrenci'))
            <!-- ZİNCİRİ KIRMA BAĞLANTISI - MOBİL MENÜ İÇİN -->
            <a href="{{ route('zinciri-kirma') }}" class="flex items-center px-4 py-3 hover:bg-red-50 {{ request()->is('zinciri-kirma') || request()->is('zinciri-kirma/*') ? 'bg-red-50 border-l-4 border-red-500 pl-3' : '' }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
                </svg>
